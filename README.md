...
IDs/names before permanent deletion. Deletion retains the exact reviewed internal
IDs and requires a successful impact check. No migration or patient-data rewrite.
...
Official release `next.73`, promoted after staging and development validation, uses the RX Pharmacy Transportation assignment
...
Official release `next.75` adds configurable Patient Tags under Reference Data.
...
bootstrap, VPN access, ports, cookies, or proxy-aware URL handling. Main
...
Official release `next.84` adds editable City Region Rules under Reference
...
The Patient screen applies the Region automatically without a browser
confirmation. If no custom rule exists, the prior fallback remains in place. No
proxy, VPN, FortiGate, cookie, `.env`, port, login/session, service bootstrap,
configured RX Action, or unrelated business-data rule is changed.

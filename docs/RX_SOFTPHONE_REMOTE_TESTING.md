...
3. Start `RxSoftphone.exe` or `Start-Softphone.cmd` as the normal Windows user who will make calls. Version 0.5.0 runs in the Windows notification area and does not require an open CMD window.
4. Double-click the RX Softphone tray icon, select **Open RX Softphone**, or open `http://127.0.0.1:5188` on that same physical computer.

...
> For the complete production-oriented procedure—including the current Cloudflare routes, illustrated screens, public Asterisk preparation, Fail2ban recovery, security notes, and acceptance checklist—use [Kasm + RX Softphone Remote Deployment Guide](KASM_RX_SOFTPHONE_REMOTE_DEPLOYMENT_GUIDE.md). This shorter file remains the compact test checklist.
...
- The approved `RxSoftphone-0.6.0-win-x64.zip` package and the Evergreen Microsoft Edge WebView2 Runtime.
...
1. With workstation-administrator approval, create `C:\Program Files\RX Softphone` so the standard Call Center user cannot alter the managed configuration or executable.
2. Extract the ZIP into that folder. Do not run the executable from inside the ZIP and do not grant the Call Center user write permission to the installation folder.
3. Start `RxSoftphone.exe` or `Start-Softphone.cmd` as the normal Windows user who will make calls. Version 0.6.0 runs in the Windows notification area and does not require an open CMD window.
4. Double-click the RX Softphone tray icon or select **Open RX Softphone**. The control panel opens in RX Softphone's own Windows window instead of an external browser. Closing its **X** hides the window back to the tray without stopping registration, relay polling, or calls.
5. Confirm the application window loads. If it shows the WebView2 recovery screen, install the Evergreen Microsoft Edge WebView2 Runtime and select **Retry**. Windows Firewall may prompt the first time; keep the API loopback-only and allow only the network access required for SIP/RTP.

To start it automatically for that Windows user, right-click the RX Softphone tray icon and check **Start with Windows**. This creates a current-user startup entry; it does not install a Windows service because SIP audio, the tray, and the protected pairing must remain in the interactive user's session. Pairing survives an application restart, Windows restart, or sleep/wake because its relay token is protected in the persistent Windows user profile. Re-pair only after the RX user changes, the administrator replaces the pairing, or the Windows profile/local pairing data is removed.

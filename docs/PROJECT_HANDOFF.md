...
- Release candidate **v4.0.0-next.42** is isolated from the published `v4.0.0-next.41` baseline for test-server validation. It adds a master-administrator RX Profile Sync screen that can search a specific active RX record, show Patient-to-RX Pharmacy/Transport differences, and apply only selected fields with RX History and Audit Log entries. It does not modify the Patient profile, workflow/delivery state, future RX creation, deleted records, or any RX record that the administrator did not select. No database migration or automatic bulk synchronization is included. It is not yet published or installed.

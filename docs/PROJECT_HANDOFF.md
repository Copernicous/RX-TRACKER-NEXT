...
- Release candidate **v4.0.0-next.43** is isolated from the published `v4.0.0-next.42` baseline for test-server validation. It corrects RX Records multi-select filter layout: selected text remains inside the control, multiple selections use a compact count, and a single long selection truncates safely. No database migration, workflow-action configuration, or business-data change is included. It is not yet published or installed.

...
- **v4.0.0-next.44** is the pending release candidate for RX Profile Sync transport reliability, master-only multi-RX selection (maximum 100 per batch), and audited sync-history CSV export. Pharmacy synchronization retains its existing update path; Patient Transport and Pharmacy Transport are explicitly saved and re-read before success. Blank Patient assignments do not erase RX values, and each changed RX receives independent RX History and Audit Log entries. No database migration, workflow-action configuration, automatic background sync, or business-data rewrite is included. The candidate must pass PostgreSQL lifecycle CI, compiled release asset verification, and test-server validation before production.

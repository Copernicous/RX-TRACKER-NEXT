...
RX Softphone 0.4.1 adds the outbound Windows relay used by Kasm and other
...
does not mark the patient Called.
...
4. Start RX Softphone 0.4.1 or later on the Windows calling computer.
...
- Install/distribute RX Softphone 0.4.1 or later to each calling workstation.
- Back up the production database and keep the previous release package.

Production verification
-----------------------

1. Confirm server.exe --v reports 3.3.0.
...
7. Verify a Call Center user cannot change a locked phone account unless an
   Administrator grants Allow setup again.

Production package
------------------

- Deploy dist/server-update-3.3.0.zip or approved dist files only.
